...
<%String msg = request.getParameter("msg");
			if (auctionLoginEnums.AUCTION_LOGIN_SUCCESS.getValue().equals(msg)) {
				out.print("alert('登录成功');");
			}
			if (auctionLoginEnums.AUCTION_LOGIN_FAIL.getValue().equals(msg)) {
				out.print("alert('用户名密码错误，请重新登录');");
			}
			if (auctionLoginEnums.AUCTION_LOGIN_VALIDATA_CODE_ERROR.getValue().equals(msg)) {
						...
						<dl>
							...
							<dd>
								<label for="name">用户名：</label> <input type="text"
									name="username01" class="inputh" value="${username}" id="name" />
							</dd>
							<dd>
								<label for="password">密 码：</label> <input type="password"
									name="userpassword01" class="inputh" value="${userpassword}"
									id="password" />
							</dd>
							<dd>
								<label class="lf" for="passwords">验证码：</label> <input
									type="text" name="inputCode" class="inputh inputs lf"
